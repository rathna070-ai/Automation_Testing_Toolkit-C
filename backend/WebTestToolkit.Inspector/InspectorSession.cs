...
public sealed class InspectorSession : IDisposable
{
    ...
    internal const int OverlayVersion = 5;
    ...
    //
    // This same set is duplicated in tests/WebTestToolkit.GeneratedTests/Support/
    // DriverContext.cs, because that project deliberately has zero project references to the
    // toolkit and so cannot share a class. ChromeOptionsParityTests reads both files and
    // fails if they drift — without it, you would record in a hardened browser and replay in
    // a bare one, which is how a suppressed dialog reappears mid-run and fails a test
    // somewhere unrelated to its cause.
    internal static void ApplyPopupSuppression(ChromeOptions options)
    {
