...
| **P23** | Fixed exporting the wrong flow, and the gaps around it. `ExportPage`/`FlowsPage` read the flow from React Router state and fell back to `SAMPLE_FLOW` — but both are top-level nav links, so arriving other than via the Flows → Export link silently substituted the built-in sample and produced a plausible document full of sample steps. The implicit fallback is gone: choosing a flow is explicit, loading the sample is a button, and the saved-flow picker P19 built for Flows is now one shared `FlowPicker` component used by both (the duplication is how Export was missed). Also: accepted edge cases now persist as flows, which finally makes P22's `edgeCaseFlows` export reachable from the UI; the step-label path dedupes the element-kind noun the way the locator-key path always has (`I click the login button button`) and keeps raw text when sanitizing would leave no letters (`$29.99` → `2999`); and the run-triage panel pre-fills Auto-heal instead of only linking to it | `frontend/src/components/FlowPicker.tsx`, `frontend/src/pages/{Export,Flows,Failures,AutoHeal}Page.tsx`, `Inspector/Capture/StepLabeler.cs` |
